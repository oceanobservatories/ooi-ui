...
@app.route('/api/uframe/get_csv/<string:stream_name>/<string:reference_designator>/<string:start_time>/<string:end_time>/<string:dpa_flag>')
def get_csv(stream_name, reference_designator, start_time, end_time, dpa_flag):
    token = get_login()
    req = requests.get(app.config['SERVICES_URL'] + '/uframe/get_csv/%s/%s/%s/%s/%s' %  (stream_name, reference_designator, start_time, end_time, dpa_flag), auth=(token, ''), stream=True)
    return Response(stream_with_context(req.iter_content(chunk_size=1024*1024*4)), headers=dict(req.headers))

@app.route('/api/uframe/get_json/<string:stream_name>/<string:reference_designator>/<string:start_time>/<string:end_time>/<string:dpa_flag>')
def get_json(stream_name, reference_designator, start_time, end_time, dpa_flag):
    token = get_login()
    req = requests.get(app.config['SERVICES_URL'] + '/uframe/get_json/%s/%s/%s/%s/%s' % (stream_name, reference_designator, start_time, end_time, dpa_flag), auth=(token, ''), stream=True)
    return Response(stream_with_context(req.iter_content(chunk_size=1024*1024*4)), headers=dict(req.headers))

@app.route('/api/uframe/get_netcdf/<string:stream_name>/<string:reference_designator>/<string:start_time>/<string:end_time>/<string:dpa_flag>')
def get_netcdf(stream_name, reference_designator, start_time, end_time, dpa_flag):
    token = get_login()
    req = requests.get(app.config['SERVICES_URL'] + '/uframe/get_netcdf/%s/%s/%s/%s/%s' %  (stream_name, reference_designator, start_time, end_time, dpa_flag), auth=(token, ''), stream=True)
    return Response(stream_with_context(req.iter_content(chunk_size=1024*1024*4)), headers=dict(req.headers))

@app.route('/api/uframe/get_profiles/<string:stream_name>/<string:reference_designator>/<string:start_time>/<string:end_time>/<string:dpa_flag>')
def get_profiles(stream_name, reference_designator, start_time, end_time, dpa_flag):
    token = get_login()
    req = requests.get(app.config['SERVICES_URL'] + '/uframe/get_profiles/%s/%s/%s/%s/%s' % (stream_name, reference_designator, start_time, end_time, dpa_flag), stream=True)
    return Response(stream_with_context(req.iter_content(chunk_size=1024*1024*4)), headers=dict(req.headers))

@app.route('/svg/plot/<string:reference_designator>/<string:stream>')
def get_plotdemo(instrument, stream):
    token = get_login()
    import time
    t0 = time.time()
    req = requests.get(app.config['SERVICES_URL'] + '/uframe/plot/%s/%s/%s/%s/%s' % (stream, ref, request.args['start_time'], request.args['end_time'], '0'), auth=(token, ''), stream=True)
    t1 = time.time()
